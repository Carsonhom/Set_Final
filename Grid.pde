public class Grid {
  ...
  public void removeSet() {
    ...
    if(cardsInPlay > 12){
      int toBeRmv = 0;
      while(toBeRmv < 3){
       int col1 = selectedLocs.get(toBeRmv).getCol();
       int row1 = selectedLocs.get(toBeRmv).getRow();
       board[col1][row1] = board[currentCols][toBeRmv];
       board[currentCols][toBeRmv] = null;
       toBeRmv++;
      }
      currentCols--;
      cardsInPlay -= 3;
  ...
  public void addCardToBoard(Card card) {
    // YOU WRITE THIS
      board[cardsInPlay % (ROWS+1)][(int)Math.floor(cardsInPlay / currentCols)] = card;
     cardsInPlay++;
  }
  ...
  public void addColumn() {
    ...
    if(grid.findSet().isEmpty()){
      score += 5;
      message = 3;
      int count = 1;
      currentCols++;
      while(count < 4){
        cardsInPlay = (count * (currentCols - 1)) + (count - 1);
        addCardToBoard(deck.deal());
        count++;
      }
      return; //<>//
    }else if(grid.findSet().size() > 0){
        score -= 5;
        message = 4;
        return;
      }
  ...
  public boolean isGameOver() {
    // YOU WRITE THIS
    if(deck.size() == 0 && grid.findSet().isEmpty()){
     return true; 
    }
    return false;
  }
